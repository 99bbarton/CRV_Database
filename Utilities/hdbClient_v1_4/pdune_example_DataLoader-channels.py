...
def createSiPMs(sid):
    return {
        ...
        'iv_curve': open("kshark-zoom-in-select.png", "rb"),
        #'iv_curve_thmb': "",                         # text
...
def createChannel(cid, chno, sid):
    return {
        ...
        'test_trace_image': open("kshark-zoom-in-select.png", "rb"),
        'bias_voltage':   1.2345678,
        'threshold':  12345,
        'dark_rate':   12.2345678,
        'gain':    123.2345678,
        'cross_talk':    1234.2345678,
        'pre_test_photo': open("kshark-zoom-in-select.png", "rb"),
...
dataLoaders = DataLoader(password, url, group="Detector Tables", table="sipms")
dataLoader = DataLoader(password, url, group="Detector Tables", table="channels")
dataLoader1 = DataLoader(password, url, group="Detector Tables", table="hoverboards")

cid = random.randrange(1000)

dataLoader1.addRow(dict(hoverboard_sn="board_{}".format(cid), procedure_document_version='v1.0.1'))
(retVal, code, text) = dataLoader1.send()
if retVal:
...
(retVal, code, text) = dataLoaders.send()
if retVal:
    print "DataLoader - Success!"
    print text
else:
...
if 1:
    ...
    row['pre_test_photo'] = open("contour_colorbar.png", "rb")
