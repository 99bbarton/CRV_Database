...
def getSNsByLength():
    ...
    print "\nEnter the length of the dicounters you wish to see plotted in meters: "
    ...
    if len(sns) == 0:
        print "No dicounters found within +/-1cm of length " + str(length) + " m"
...
#------------------------------------------------------------------------------------------------------------
##Function to retrieve a list of serial numbers by the crate number where they are stored
def getSNsInCrate():
    global database
    
    sns = []

    num = raw_input("\nEnter the crate number: ")
    desiredLoc = "Crate " + num
        
    print "Querying..."
    snsLocs = DatabaseQueryTool.query(database,"di_counters","di_counter_id,location")

    for pair in snsLocs:
        components = pair.split(",")
        if len(components[1]) > 0: #If dicounter has data in the location field
            if components[1].find(desiredLoc) >= 0: #If crate number is found in location field
                sns.append(int(components[0][3:]))

    if len(sns) == 0:
        print 'No dicounters found matching location "Crate ' + num + '"'

    return sns

#----------------------------------------------------------------------------------------------    
##Function to retrieve a list of serial numbers of a specific length that are not in modules yet
def getSNsNotInModule():
    global database
    ...
    print "Querying..."
    snsMods = DatabaseQueryTool.query(database,"di_counters","di_counter_id,module_id",fetchCondition)

    for result in snsMods:
        components = result.split(",")
        print components
        if components[1].find("crvmod") < 0: #If no module id
            sns.append(int(components[0][3:]))

    if len(sns) == 0:
        print "No dicounters found within +/-1cm of length " + str(length) + " m, not already in modules"
...
def checkTransmissionQA_db(snList):
    ...
    for sn in snList:
        ...
        lines = DatabaseQueryTool.query(database,"di_counter_tests","di_counter_id,light_source,sipm_location,distance,current_amps,comments",fetchCondition,"di_counter_id,light_source,sipm_location,distance,current_amps,comments")
...
def main(args):
    ...
    if mode == 1:
        ...
        if checkDB:
            ...
            print 'Enter "C" for dicounters stored in a specific crate'
            print 'ENter "A" for dicounters of a specific length not built into a module'
